...
public interface AsyncMap<K, V> {
   ...
   *
   * @param k the key
   * @param resultHandler - this will be called some time later with the async
   * result.
   */
   ...
   * @param k the key
   * @param v the value
   * @param completionHandler - this will be called some time later to signify
   * the value has been put
   */
  void put(K k, V v, Handler<AsyncResult<Void>> completionHandler);

  /**
   * Like {@link #put} but specifying a time to live for the entry. Entry will
  ...
  /**
   * Put the entry only if there is no entry with the key already present. If
   * key already present then the existing value will be returned to the
  ...
  /**
   * Link {@link #putIfAbsent} but specifying a time to live for the entry.
   ...
   *
   * @param k the key
   * @param resultHandler - this will be called some time later to signify the
