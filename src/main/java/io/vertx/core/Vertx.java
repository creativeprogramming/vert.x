 ...
 * {@link #vertx(io.vertx.core.VertxOptions)} and
 * {@link #clusteredVertx(io.vertx.core.VertxOptions, Handler)}.
 * <p>
...
public interface Vertx extends Measured {
   ...
   * <p>
   * The instance is created asynchronously and the resultHandler is called with
  ...
  /**
   * Get the filesystem object. There is a single instance of FileSystem per
   * Vertx instance.
   *
  ...
  /**
   * Get the event bus object. There is a single instance of EventBus per Vertx
   * instance.
   *
  ...
  /**
   * Create a DNS client to connect to a DNS server at the specified host and
   * port
   *
   * @param port the port
   * @param host the host
  ...
  /**
   * Get the shared data object. There is a single instance of SharedData per
   * Vertx instance.
   *
  ...
  /**
   * Set a one-shot timer to fire after {@code delay} milliseconds, at which
   * point {@code handler} will be called with the id of the timer.
   *
   * @param delay the delay in milliseconds, after which the timer will fire
   * @param handler the handler that will be called with the timer ID when the
   * timer fires
   ...
   * {@code delay} milliseconds after the {@link ReadStream#handler} has been
   * called.
   *
   * @param delay the delay in milliseconds, after which the timer will fire
  ...
  /**
   * Set a periodic timer to fire every {@code delay} milliseconds, at which
   * point {@code handler} will be called with the id of the timer.
   *
   *
   * @param delay the delay in milliseconds, after which the timer will fire
   ...
   * @return true if the timer was successfully cancelled, or false if the timer
   * does not exist.
   */
   ...
   * <p>
   * The actual close is asynchronous and may not complete until after the call
   * has returned.
   */
  void close();

  /**
   * Like {@link #close} but the completionHandler will be called when the close
   * is complete
   *
   * @param completionHandler The handler will be notified when the close is
   * complete.
   */
   ...
   * <p>
   * The actual deploy happens asynchronously and may not complete until after
  ...
  /**
   * Like {@link #deployVerticle(Verticle)} but the completionHandler will be
   * notified when the deployment is complete.
   * <p>
   * If the deployment is successful the result will contain a string
   * representing the unique deployment ID of the deployment.
   * <p>
   * This deployment ID can subsequently be used to undeploy the verticle.
   *
   * @param verticle the verticle instance to deploy
   * @param completionHandler a handler which will be notified when the
  ...
  /**
   * Like {@link #deployVerticle(Verticle)} but
  ...
  /**
   * Like {@link #deployVerticle(Verticle, Handler)} but
   * {@link io.vertx.core.DeploymentOptions} are provided to configure the
   * deployment.
   *
   * @param verticle the verticle instance to deploy
   ...
   * <p>
   * Given the name, Vert.x selects a {@link VerticleFactory} instance to use to
   * instantiate the verticle.
   * <p>
  ...
  /**
   * Like {@link #deployVerticle(String)} but the completionHandler will be
   * notified when the deployment is complete.
   * <p>
   * If the deployment is successful the result will contain a String
   * representing the unique deployment ID of the deployment.
  ...
  /**
   * Like {@link #deployVerticle(String, Handler)} but
   * {@link io.vertx.core.DeploymentOptions} are provided to configure the
   * deployment.
   *
   * @param name the name
   ...
   * <p>
   * The actual undeployment happens asynchronously and may not complete until
  ...
  /**
   * Register a {@code VerticleFactory} that can be used for deploying Verticles
   ...
   * <p>
   * In the {@code blockingCodeHandler} the current context remains the original
   * context and therefore any task scheduled in the {@code blockingCodeHandler}
  ...
  /**
   * Like {@link #executeBlocking(Handler, boolean, Handler)} called with
   * ordered = true.
   */
  ...
  /**
   * Like {@link #createSharedWorkerExecutor(String, int)} but with the
   * {@link VertxOptions#setWorkerPoolSize} {@code poolSize}.
   */
  ...
  /**
   * Create a named worker executor, the executor should be closed when it's not
   * needed anymore to release resources.<p/>
   *
   * This method can be called mutiple times with the same {@code name}.
   * Executors with the same name will share the same worker pool. The worker
   * pool size and max execute time are set when the worker pool is created and
   * won't change after.<p>
   *
   * The worker pool is released when all the {@link WorkerExecutor} sharing the
   * same name are closed.
   *
  ...
  /**
   * Set a default exception handler for {@link Context}, set on
   * {@link Context#exceptionHandler(Handler)} at creation.
   *
