...
/**
 * Interface defining a command-line interface (in other words a command such as
 * 'run', 'ls'...). This interface is polyglot to ease reuse such as in Vert.x
 * Shell.
 * <p/>
 * A command line interface has a name, and defines a set of options and
 * arguments. Options are key-value pair such as {@code -foo=bar} or
 * {@code -flag}. The supported formats depend on the used parser. Arguments are
 * unlike options raw values. Options are defined using {@link Option}, while
 * argument are defined using {@link Argument}.
 * <p/>
 * Command line interfaces also define a summary and a description. These
 * attributes are used in the usage generation . To disable the help generation,
 * set the {@code hidden} attribute to {@code true}.
 * <p/>
 * Command Line Interface object does not contains "value", it's a model. It
 * must be evaluated by a parser that returns a {@link CommandLine} object
 * containing the argument and option values.
 *
...
public interface CLI {
  ...
  /**
   * Creates an instance of {@link CLI} from the given Java class. It
   * instantiates the {@link CLI} object from the annotations used in the class.
   *
   ...
   *
   * @return {@code true} if the current {@link CLI} is hidden, {@link false}
   * otherwise
   */
  boolean isHidden();

  /**
   * Sets whether or not the current instance of {@link CLI} must be hidden.
   * Hidden CLI are not listed when displaying usages / help messages. In other
   * words, hidden commands are for power user.
   *
  ...
  /**
   * Adds a set of options. Unlike {@link #setOptions(List)}}, this method does
   * not remove the existing options. The given list is appended to the existing
   * list.
   *
  ...
  /**
   * Adds a set of arguments. Unlike {@link #setArguments(List)}, this method
   * does not remove the existing arguments. The given list is appended to the
   * existing list.
   *
  ...
  /**
   * Gets an {@link Option} based on its name (short name, long name or argument
   * name).
   *
  ...
  /**
   * Removes an option identified by its name. This method does nothing if the
  ...
  /**
   * Removes an argument identified by its index. This method does nothing if
