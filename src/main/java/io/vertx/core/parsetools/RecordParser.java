...
/**
 * A helper class which allows you to easily parse protocols which are delimited
 * by a sequence of bytes, or fixed size records.
 * <p>
 * Instances of this class take as input {@link io.vertx.core.buffer.Buffer}
 ...
 * </pre> Instances of this class can be changed between delimited mode and
 * fixed size record mode on the fly as individual records are read, this allows
 * you to parse protocols where, for example, the first 5 records might all be
 * fixed size (of potentially different sizes), followed by some delimited
