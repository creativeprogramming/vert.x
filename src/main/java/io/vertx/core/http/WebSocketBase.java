...
public interface WebSocketBase extends ReadStream<Buffer>, WriteStream<Buffer> {
  ...
  /**
   * When a {@code Websocket} is created it automatically registers an event
   * handler with the event bus - the ID of that handler is given by this
   * method.
   * <p>
   * Given this ID, a different event loop can send a binary frame to that event
   ...
   * This allows you to write data to other WebSockets which are owned by
  ...
  /**
   * When a {@code Websocket} is created it automatically registers an event
   * handler with the eventbus, the ID of that handler is given by
   * {@code textHandlerID}.
   * <p>
   * Given this ID, a different event loop can send a text frame to that event
   * handler using the event bus and that buffer will be received by this
   * instance in its own event loop and written to the underlying connection.
   * This allows you to write data to other WebSockets which are owned by
   * different event loops.
  ...
  /**
   * Writes a (potentially large) piece of binary data to the connection. This
   * data might be written as multiple frames if it exceeds the maximum
  ...
  /**
   * Set a frame handler on the connection. This handler will be called when
   * frames are read on the connection.
   *
   * @param handler the handler
   * @return a reference to this, so the API can be used fluently
   */
