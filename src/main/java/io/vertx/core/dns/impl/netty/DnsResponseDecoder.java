...
public class DnsResponseDecoder extends MessageToMessageDecoder<DatagramPacket> {
  ...
  /**
   * Retrieves a domain name given a buffer containing a DNS packet. If the name
   * contains a pointer, the position of the buffer will be set to directly
   * after the pointer's index after the name has been read.
   *
  ...
  /**
   * Decodes a response from a {@link io.netty.channel.socket.DatagramPacket}
   * containing a {@link io.netty.buffer.ByteBuf} with a DNS packet. Responses
   * are sent from a DNS server to a client in response to a query. This method
   * writes the decoded response to the specified {@link java.util.List} to be
   ...
   * @param packet the message being decoded, a
   * {@link io.netty.channel.socket.DatagramPacket} containing a DNS packet
   * @param out the {@link java.util.List} to which decoded messages should be
   * added
   * @throws Exception
   */
  @Override
