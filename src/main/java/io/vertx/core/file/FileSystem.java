 ...
 * <p>
 * A (potential) blocking and non blocking version of each operation is
 * provided.
 * <p>
 * The non blocking versions take a handler which is called when the operation
 * completes or an error occurs.
 * <p>
 * The blocking versions are named {@code xxxBlocking} and return the results,
 * or throw exceptions directly. In many cases, depending on the operating
 * system and file system some of the potentially blocking operations can return
 * quickly, which is why we provide them, but it's highly recommended that you
 * test how long they take to return in your particular application before using
...
public interface FileSystem {
   ...
   * Copy a file from the path {@code from} to path {@code to}, asynchronously.
   * <p>
   * If {@code recursive} is {@code true} and {@code from} represents a
   * directory, then the directory and its contents will be copied recursively
   * to the destination {@code to}.
   * <p>
  ...
  /**
   * Blocking version of
   * {@link #copyRecursive(String, String, boolean, Handler)}
   */
  @Fluent
  ...
  /**
   * Truncate the file represented by {@code path} to length {@code len} in
   * bytes, asynchronously.
   * <p>
   * The operation will fail if the file does not exist or {@code len} is less
   * than {@code zero}.
   *
  ...
  /**
   * Change the permissions on the file represented by {@code path} to
   * {@code perms}, asynchronously.
   * <p>
   * The permission String takes the form rwxr-x--- as specified
   * <a href="http://download.oracle.com/javase/7/docs/api/java/nio/file/attribute/PosixFilePermissions.html">here</a>.
   *
   * @param path the path to the file
  ...
  /**
   * Change the permissions on the file represented by {@code path} to
   * {@code perms}, asynchronously.<p>
   * The permission String takes the form rwxr-x--- as specified in
   * {<a href="http://download.oracle.com/javase/7/docs/api/java/nio/file/attribute/PosixFilePermissions.html">here</a>}.
   * <p>
   * If the file is directory then all contents will also have their permissions
   * changed recursively. Any directory permissions will be set to
   * {@code dirPerms}, whilst any normal file permissions will be set to
   * {@code perms}.
   *
   * @param path the path to the file
   * @param perms the permissions string
  ...
  /**
   * Blocking version of
   * {@link #chmodRecursive(String, String, String, Handler)}
   */
  @Fluent
  FileSystem chmodRecursiveBlocking(String path, String perms, String dirPerms);

  /**
   * Change the ownership on the file represented by {@code path} to
   * {@code user} and {code group}, asynchronously.
   *
   * @param path the path to the file
   * @param user the user name, {@code null} will not change the user name
   * @param group the user group, {@code null} will not change the user group
   * name
   * @param handler the handler that will be called on completion
   * @return a reference to this, so the API can be used fluently
   */
  ...
  /**
   * Create a hard link on the file system from {@code link} to
   * {@code existing}, asynchronously.
   *
   * @param link the link
  ...
  /**
   * Create a symbolic link on the file system from {@code link} to
   * {@code existing}, asynchronously.
   *
   * @param link the link
   * @param existing the link destination
  ...
  /**
   * Unlinks the link on the file system represented by the path {@code link},
   * asynchronously.
   *
   * @param link the link
   * @param handler the handler that will be called on completion
   ...
   * {@code link} points to, asynchronously.
   *
   * @param link the link
   * @param handler the handler that will be called on completion
   * @return a reference to this, so the API can be used fluently
   ...
   * Deletes the file represented by the specified {@code path}, asynchronously.
   * <p>
   * If the path represents a directory and {@code recursive = true} then the
   * directory and its contents will be deleted recursively.
   *
   * @param path path to the file
  ...
  /**
   * Create the directory represented by {@code path} and any non existent
  ...
  /**
   * Read the contents of the directory specified by {@code path},
   * asynchronously.
   * <p>
   * The result is an array of String representing the paths of the files inside
   * the directory.
   *
   * @param path path to the file
   * @param handler the handler that will be called on completion
   ...
   * asynchronously.
   * <p>
   * The parameter {@code filter} is a regular expression. If {@code filter} is
   * specified then only the paths that match @{filter}will be returned.
   * <p>
   * The result is an array of String representing the paths of the files inside
   * the directory.
   *
  ...
  /**
   * Reads the entire file as represented by the path {@code path} as a
   * {@link Buffer}, asynchronously.
   * <p>
   * Do not user this method to read very large files or you risk running out of
   * available RAM.
   *
   * @param path path to the file
   * @param handler the handler that will be called on completion
  ...
  /**
   * Creates the file, and writes the specified {@code Buffer data} to the file
   * represented by the path {@code path}, asynchronously.
   *
   * @param path path to the file
   * @param handler the handler that will be called on completion
   * @return a reference to this, so the API can be used fluently
   ...
   * <p>
   * The file is opened for both reading and writing. If the file does not
  ...
  /**
   * Blocking version of
   * {@link #open(String, io.vertx.core.file.OpenOptions, Handler)}
   */
  ...
  /**
   * Creates an empty file with the specified {@code path} and permissions
   * {@code perms}, asynchronously.
   *
   * @param path path to the file
   * @param perms the permissions string
  ...
  /**
   * Determines whether the file as specified by the path {@code path} exists,
   * asynchronously.
   *
   * @param path path to the file
   * @param handler the handler that will be called on completion
