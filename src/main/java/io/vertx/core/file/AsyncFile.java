 ...
 * {@link io.vertx.core.streams.WriteStream}. This allows the data to be pumped
 * to and from other streams, e.g. an
 * {@link io.vertx.core.http.HttpClientRequest} instance, using the
 * {@link io.vertx.core.streams.Pump} class
 *
...
public interface AsyncFile extends ReadStream<Buffer>, WriteStream<Buffer> {
  ...
  /**
   * Close the file. The actual close happens asynchronously. The handler will
   * be called when the close is complete, or an error occurs.
   *
  ...
  /**
   * Write a {@link io.vertx.core.buffer.Buffer} to the file at position
   * {@code position} in the file, asynchronously.
   * <p>
   * If {@code position} lies outside of the current size of the file, the file
   * will be enlarged to encompass it.
   * <p>
   * When multiple writes are invoked on the same file there are no guarantees
   * as to order in which those writes actually occur
   * <p>
   * The handler will be called when the write is complete, or if an error
   * occurs.
   *
  ...
  /**
   * Reads {@code length} bytes of data from the file at position
   * {@code position} in the file, asynchronously.
   * <p>
   * The read data will be written into the specified {@code Buffer buffer} at
   * position {@code offset}.
   * <p>
   * If data is read past the end of the file then zero bytes will be read.<p>
   * When multiple reads are invoked on the same file there are no guarantees as
   * to order in which those reads actually occur.
   * <p>
   * The handler will be called when the close is complete, or if an error
   * occurs.
   *
   ...
   * <p>
   * If the file was opened with {@code flush} set to {@code true} then calling
  ...
  /**
   * Same as {@link #flush} but the handler will be called when the flush is
  ...
  /**
   * Sets the position from which data will be read from when using the file as
   * a {@link io.vertx.core.streams.ReadStream}.
   *
   ...
   * Changing this value will impact how much the data will be read at a time
