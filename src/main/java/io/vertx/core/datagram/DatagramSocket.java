...
/**
 * A datagram socket can be used to send {@link DatagramPacket}'s to remote
 * datagram servers and receive {@link DatagramPacket}s .
 * <p>
 * Usually you use a datagram socket to send UDP over the wire. UDP is
...
public interface DatagramSocket extends ReadStream<DatagramPacket>, Measured {
  ...
  /**
   * Write the given {@link io.vertx.core.buffer.Buffer} to the
   * {@link io.vertx.core.net.SocketAddress}. The {@link io.vertx.core.Handler}
   * will be notified once the write completes.
   *
   * @param packet the {@link io.vertx.core.buffer.Buffer} to write
   * @param port the host port of the remote peer
   * @param host the host address of the remote peer
   * @param handler the {@link io.vertx.core.Handler} to notify once the write
   * completes.
  ...
  /**
   * Returns a {@link io.vertx.core.datagram.PacketWritestream} able to send
   * {@link Buffer} to the {@link io.vertx.core.net.SocketAddress}.
   *
  ...
  /**
   * Write the given {@link String} to the
   * {@link io.vertx.core.net.SocketAddress} using UTF8 encoding. The
   * {@link Handler} will be notified once the write completes.
   *
  ...
  /**
   * Write the given {@link String} to the
   * {@link io.vertx.core.net.SocketAddress} using the given encoding. The
   * {@link Handler} will be notified once the write completes.
   *
   * @param str the {@link String} to write
  ...
  /**
   * Closes the {@link io.vertx.core.datagram.DatagramSocket} implementation
   * asynchronous and notifies the handler once done.
   *
  ...
  /**
   * Closes the {@link io.vertx.core.datagram.DatagramSocket}. The close itself
   * is asynchronous.
   */
  void close();

  /**
   * Return the {@link io.vertx.core.net.SocketAddress} to which this
   * {@link io.vertx.core.datagram.DatagramSocket} is bound.
   *
  ...
  /**
   * Joins a multicast group and listens for packets send to it. The
   * {@link Handler} is notified once the operation completes.
   *
  ...
  /**
   * Joins a multicast group and listens for packets send to it on the given
   * network interface. The {@link Handler} is notified once the operation
   * completes.
   *
   * @param multicastAddress the address of the multicast group to join
   * @param networkInterface the network interface on which to listen for
   * packets.
   * @param source the address of the source for which we will listen for
   * multicast packets
   * @param handler then handler to notify once the operation completes
   * @return a reference to this, so the API can be used fluently
   */
  ...
  /**
   * Leaves a multicast group and stops listening for packets send to it. The
   * {@link Handler} is notified once the operation completes.
   *
  ...
  /**
   * Leaves a multicast group and stops listening for packets send to it on the
   * given network interface. The {@link Handler} is notified once the operation
   * completes.
   *
   * @param multicastAddress the address of the multicast group to join
   * @param networkInterface the network interface on which to listen for
   ...
   * {@link Handler} once the operation completes.
   *
   * @param multicastAddress the address for which you want to block the source
   * address
   * @param sourceToBlock the source address which should be blocked. You will
   * not receive an multicast packets for it anymore.
   * @param handler the handler to notify once the operation completes
   * @return a reference to this, so the API can be used fluently
   */
  @Fluent
  ...
  /**
   * Block the given address for the given multicast address on the given
   * network interface and notifies the {@link Handler} once the operation
   * completes.
   *
   * @param multicastAddress the address for which you want to block the source
   * address
   * @param networkInterface the network interface on which the blocking should
   * occur.
   * @param sourceToBlock the source address which should be blocked. You will
   * not receive an multicast packets for it anymore.
   * @param handler the handler to notify once the operation completes
  ...
  /**
   * Start listening on the given port and host. The handler will be called when
