 ...
 * pump data from a {@code ReadStream} to it.
 *
...
public interface WriteStream<T> extends StreamBase {
  ...
  /**
   * Set the maximum size of the write queue to {@code maxSize}. You will still
   * be able to write to the stream even if there is more than {@code maxSize}
   * items in the write queue. This is used as an indicator by classes such as
   * {@code Pump} to provide flow control.
   * <p/>
   * The value is defined by the implementation of the stream, e.g in bytes for
   * a {@link io.vertx.core.net.NetSocket}, the number of
   * {@link io.vertx.core.eventbus.Message} for a
  ...
  /**
   * This will return {@code true} if there are more bytes in the write queue
   * than the value set using {@link
   ...
   * handler will be called when the write queue is ready to accept buffers
   * again. See {@link Pump} for an example of this being used.
   * <p/>
   * The stream implementation defines when the drain handler, for example it
   * could be when the queue size has been reduced to {@code maxSize / 2}.
   *
