...
/**
 * A Vert.x event-bus is a light-weight distributed messaging system which
 * allows different parts of your application, or different applications and
 * services to communicate with each in a loosely coupled way.
 * <p>
 * An event-bus supports publish-subscribe messaging, point-to-point messaging
 * and request-response messaging.
 * <p>
 * Message delivery is best-effort and messages can be lost if failure of all or
 * part of the event bus occurs.
 * <p>
...
public interface EventBus extends Measured {
   ...
   * Sends a message.
   * <p>
   * The message will be delivered to at most one of the handlers registered to
   * the address.
   *
   ...
   * @param address the address to send it to
   * @param message the message, may be {@code null}
   * @param replyHandler reply handler will be called when any reply from the
   * recipient is received, may be {@code null}
   * @return a reference to this, so the API can be used fluently
   */
  @Fluent
  ...
  /**
   * Like {@link #send(String, Object)} but specifying {@code options} that can
   * be used to configure the delivery.
   *
   * @param address the address to send it to
   * @param message the message, may be {@code null}
  ...
  /**
   * Like {@link #consumer(String, Handler)} but the address won't be propagated
   * across the cluster.
   *
   * @param address the address that will register it at
   * @param handler the handler that will process the received messages
   ...
   * specified type on the event bus without the codec having to be specified in
  ...
  /**
   * Close the event bus and release any resources held. This would not normally
  ...
  /**
   * Add an interceptor that will be called whenever a message is sent from
   * Vert.x
   *
