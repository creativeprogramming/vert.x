...
/**
 * A message codec allows a custom message type to be marshalled across the
 * event bus.
 * <p>
 * Usually the event bus only allows a certain set of message types to be sent
 * across the event bus, including primitive types, boxed primitive types, {@code byte[]}, {@link io.vertx.core.json.JsonObject}, {@link io.vertx.core.json.JsonArray},
 * {@link io.vertx.core.buffer.Buffer}.
 * <p>
 * By specifying a message codec you can pass any other type across the event
 * bus, e.g. POJOs.
 * <p>
 * With a message codec the type sent <i>does not</i> have to be the same as the
 * type received, e.g. you could send a {@code Buffer} and have it be received
 * as a {@code JsonObject}.
 * <p>
 *
 * <b>Instances of this class must be thread-safe as they may be used
 * concurrently by different threads.</b>
 *
...
public interface MessageCodec<S, R> {
   ...
   *
   * @param pos the position in the buffer where the message should be read
   * from.
   ...
   * called to transform the message from the sent type S to the received type R
   *
