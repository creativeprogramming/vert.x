 ...
 *
 * In order for HA to work correctly, all implementations of this interface MUST
 * be implemented such that:
 *
 * 1. Whenever a node joins or leaves the cluster the registered NodeListener
 * (if any) MUST be called with the appropriate join or leave event. 2. For all
 * nodes that are part of the cluster, the registered NodeListener MUST be
 * called with the exact same sequence of join and leave events on all nodes. 3.
 * For any particular join or leave event that is handled in any NodeListener,
 * anywhere in the cluster, the List of nodes returned by getNodes must be
 * identical. 4. All of the methods in the implementation must return
 * immediately, i.e. they must not block while the operation is being executed.
 * If the underlying implementation does block, then
 * {@link io.vertx.core.Vertx#executeBlocking} should be used to run the
 * operation on a worker.
 *
